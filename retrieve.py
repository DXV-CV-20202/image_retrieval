...
cifar10_test_json = './data/cifar-10/test.json'
coil100_test_json = './data/coil-100/test.json'
caltech101_test_json = './data/caltech-101/test.json'

def main(
    db_cfg_path='./config/database.json',
    connect_name='mongodb',
    extr_cfg_path='./config/feature_extractor.json',
    list_features={'HuMoments', 'HOG_CIFAR_default'},
    testset_path = coil100_test_json
    ):
    ...
    for des in testset_des:
        # Sửa đoạn này để đọc riêng cho từng loại dataset
        image = read_image_from_config(des['image_path'])
