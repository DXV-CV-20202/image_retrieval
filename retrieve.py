...
class MatchApp:
    def __init__(
        ...
        self.matcher = KDTreeMatcher(list_features=list_features, extractors=extractors, collection=collection, metric=metric)


    def do_match(self, image_path):
        image = cv2.imread(image_path)
        return self.matcher.match(image)


    def do_test(self, testset_path = './data/cifar-10/test.json'):
